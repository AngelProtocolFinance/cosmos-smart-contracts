...
use angel_core::msgs::registrar::{
    ConfigResponse, InstantiateMsg, NetworkConnectionResponse, StrategyDetailResponse,
    UpdateConfigMsg,
};
use angel_core::msgs::registrar_partial::{ExecuteMsg, QueryMsg};
...
fn update_config() {
    ...
    let update_config_message = UpdateConfigMsg {
        treasury: Some(REVIEW_TEAM.to_string()),
    ...
    let res = query(deps.as_ref(), mock_env(), QueryMsg::Config {}).unwrap();
    let config_response: ConfigResponse = from_binary(&res).unwrap();
    assert_eq!(REVIEW_TEAM, config_response.treasury);
}
