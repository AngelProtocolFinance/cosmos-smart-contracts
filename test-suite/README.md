...
### Run only the setup of contracts
...
Populate the `localConfig.ts` file, found in the root folder, with the appropriate Accounts wasm storage code ID & all Smart Contract addresses generated by: 
- a full run with: `ts-node main.ts` 
- a setup only run with: `ts-node setup.ts` 

Then you can simply run/re-run only the tests for this batch of Contracts using:
```bash
cd test-suite/scripts
npm install
ts-node tests.ts
