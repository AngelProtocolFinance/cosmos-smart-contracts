...
import { sendTransaction } from "../../utils/helpers/juno";
...
async function generateMerkleRoot(): Promise<string> {
  ...
  try {
    file1 = readFileSync(
      path.resolve(__dirname, "./airdrop/testdata/airdrop_stakers_list.json"),
      "utf-8"
    );
    file2 = readFileSync(
      path.resolve(
        __dirname,
        "./airdrop/testdata/airdrop_delegators_list.json"
      ),
      "utf-8"
    );
  } catch (e) {
  ...
  const delegators: Array<{ address: string; amount: string }> =
    JSON.parse(file2);
