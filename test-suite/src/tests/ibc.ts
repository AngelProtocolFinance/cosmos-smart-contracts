...
// IBC Resources
import { localibc, junod, terrad } from "../utils/config/localIbcConstants";
import {
  customFundAccount,
  customSigningClient,
...
export async function testExecuteIBC(ibc: {
  ...
  junoApTeamSigner = await customSigningCosmWasmClient(
    junod,
  ...
  const link = await Link.createWithExistingConnections(
    ...
    nodeB,
    localibc.conns.juno,
    localibc.conns.terra
  );
  // await testIbcQuery(link);

  const junoCharity1Signer = await customSigningCosmWasmClient(
    junod,
    localjuno.mnemonicKeys.charity1
  );
...
async function testIbcQuery(link: Link) {
    ...
    junoIcaController,
    {
      ibc_query: {
        ...
        msgs: [
          {
            smart: {
              msg: toBinary({ list_accounts: {} }),
              contract_addr: terraIcaHost,
            },
          },
        ],
      },
  ...
  const ibcQueryResult = await junoApTeamSigner.sign.queryContractSmart(
    junoIcaController,
    {
      latest_query_result: {
        channel_id: channelId,
