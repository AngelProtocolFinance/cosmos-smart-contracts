...
class Airdrop {
  ...
  public verify(
    ...
    proof.forEach((proofElem) => {
      ...
      if (hashBuf < proofBuf) {
        hashBuf = Buffer.from(sha256(Buffer.concat([hashBuf, proofBuf], totalLength).toString()).toString());
      } else {
        hashBuf = Buffer.from(sha256(Buffer.concat([proofBuf, hashBuf], totalLength).toString()).toString());
      }
    });
