...
import { sendTransaction, sendTransactionWithFunds } from "../../../utils/juno/helpers";
...
export async function testSendDepositToGiftcards(
    ...
    const res = await sendTransactionWithFunds(juno, apTeam, giftcards, { deposit: { to_address: undefined } }, [coin]);
...
export async function testQueryGiftcardsConfig(juno: SigningCosmWasmClient, giftcards: string): Promise<void> {
