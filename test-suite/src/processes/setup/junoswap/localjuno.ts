...
export async function setupJunoSwap(
    ...
    const factoryCodeId = await storeCode(juno, apTeam, `${wasm_path.junoswap}/junoswap_factory.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${factoryCodeId}`);

    process.stdout.write("Uploading JunoSwap pair Wasm");
    const pairCodeId = await storeCode(juno, apTeam, `${wasm_path.junoswap}/junoswap_pair.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${pairCodeId}`);

    process.stdout.write("Uploading JunoSwap token Wasm");
    const tokenCodeId = await storeCode(juno, apTeam, `${wasm_path.junoswap}/junoswap_token.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${tokenCodeId}`);

    process.stdout.write("Uploading JunoSwap router Wasm");
    const routerCodeId = await storeCode(juno, apTeam, `${wasm_path.junoswap}/junoswap_router.wasm`);
    ...
    const pairResult = await sendTransaction(
        juno,
        apTeam,
        "factoryContract",
        {
    ...
    console.log(chalk.green(" Done!"), `${chalk.blue("contractAddress")}=${result.liquidity_token}`);

    // send liquidity to the new Pair contract for swaps
    process.stdout.write("Provide liquidity to the new Pair contract @ ratio of 0.05 JUNO per HALO");
