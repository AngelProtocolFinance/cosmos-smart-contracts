...
export async function setupLoopSwap(
    ...
    const tokenCodeId = await storeCode(juno, apTeam, `${wasm_path.loopswap}/loopswap_token.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${tokenCodeId}`);

    process.stdout.write("Uploading LoopSwap Pair Wasm");
    const pairCodeId = await storeCode(juno, apTeam, `${wasm_path.loopswap}/loopswap_pair.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${pairCodeId}`);

    process.stdout.write("Uploading LoopSwap Factory Wasm");
    const factoryCodeId = await storeCode(juno, apTeam, `${wasm_path.loopswap}/loopswap_factory.wasm`);
    console.log(chalk.green(" Done!"), `${chalk.blue("codeId")}=${factoryCodeId}`);

    process.stdout.write("Uploading LoopSwap Farming Wasm");
    const farmingCodeId = await storeCode(juno, apTeam, `${wasm_path.loopswap}/loopswap_farming.wasm`);
    ...
    console.log(chalk.green(" Done!"), `${chalk.blue("contractAddress")}=${loopJunoPairLP}`);

    // send liquidity to the new Swap pool contract for swaps
    process.stdout.write("Add liquidity to the new LOOP/JUNO pair contract @ ratio of 0.5 JUNO per LOOP");
    ...
    console.log(chalk.green(" Done!"), `${chalk.blue("contractAddress")}=${maloJunoPairLP}`);

    // send liquidity to the new MALO/JUNO pool contract for swaps
    process.stdout.write("Add liquidity to the new MALO/JUNO pair contract @ ratio of 0.5 JUNO per MALO");
    ...
    console.log(chalk.green(" Done!"), `${chalk.blue("contractAddress")}=${kaloJunoPairLP}`);

    // send liquidity to the new KALO/JUNO pool contract for swaps
    process.stdout.write("Add liquidity to the new KALO/JUNO pair contract @ ratio of 0.5 JUNO per KALO");
    ...
    console.log(chalk.green(" Done!"), `${chalk.blue("contractAddress")}=${maloKaloPairLP}`);

    // send liquidity to the new MALO/KALO pool contract for swaps
    process.stdout.write("Add liquidity to the new MALO/KALO pair contract @ ratio of 1 MALO per KALO");
