...
import { setupIBC } from "../../setup/ibc";
import { testExecuteIBC } from "../../tests/ibc";
...
async function initialize() {
  ...
  console.log(
    `Using ${chalk.cyan(junoIcaController)} as Juno ica controller contract`
  );
  console.log(
    `Using ${chalk.cyan(
      `wasm.${junoIcaController}`
    )} as Juno ica controller Port`
  );
  console.log(`Using ${chalk.cyan(junoIcaHost)} as Juno ica host contract`);
  console.log(
    `Using ${chalk.cyan(`wasm.${junoIcaHost}`)} as Juno ica host Port`
  );

  console.log(
    `Using ${chalk.cyan(terraIcaController1)} as Terra ica controller1 contract`
  );
  console.log(
    `Using ${chalk.cyan(
      `wasm.${terraIcaController1}`
    )} as Terra ica controller1 Port`
  );
  console.log(
    `Using ${chalk.cyan(terraIcaController2)} as Terra ica controller2 contract`
  );
  console.log(
    `Using ${chalk.cyan(
      `wasm.${terraIcaController2}`
    )} as Terra ica controller2 Port`
  );
  console.log(`Using ${chalk.cyan(terraIcaHost)} as Terra ica host contract`);
  console.log(
    `Using ${chalk.cyan(`wasm.${terraIcaHost}`)} as Terra ica host Port`
  );
}
