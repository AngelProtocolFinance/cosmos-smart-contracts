...
// import { migrateHalo } from "../../migrate/halo";

import { setupCore } from "../../setup/core/testnet";
import {
  setupCharityEndowments,
  setupNormalEndowments,
} from "../../setup/endowments/endowments";
import { setupGiftcards } from "../../setup/accessories/giftcards";
import { setupLoopSwap } from "../../setup/loopswap/localjuno";
import { setupMockVaults } from "../../setup/vaults/mock-vault";
import { setupLoopVaults } from "../../setup/vaults/loop";
// import { setupJunoSwap } from "../../setup/junoswap/localjuno";
// import { setupHalo } from "../../setup/halo";

import { testExecute } from "../../tests/testnet";
import {
  endowListCharity,
  endowListNormal,
} from "../../setup/endowments/testnetEndowLists";
...
async function initialize() {
  ...
  apTeam2 = await DirectSecp256k1HdWallet.fromMnemonic(
    config.mnemonicKeys.apTeam2,
    {
      prefix: config.networkInfo.walletPrefix,
    }
  );
  apTeam3 = await DirectSecp256k1HdWallet.fromMnemonic(
    config.mnemonicKeys.apTeam3,
    {
      prefix: config.networkInfo.walletPrefix,
    }
  );
  apTreasury = await DirectSecp256k1HdWallet.fromMnemonic(
    config.mnemonicKeys.apTreasury,
    {
      prefix: config.networkInfo.walletPrefix,
    }
  );
  charity1 = await DirectSecp256k1HdWallet.fromMnemonic(
    config.mnemonicKeys.charity1,
    {
      prefix: config.networkInfo.walletPrefix,
    }
  );
  charity2 = await DirectSecp256k1HdWallet.fromMnemonic(
    config.mnemonicKeys.charity2,
    {
      prefix: config.networkInfo.walletPrefix,
    }
  );
  ...
  tca = await DirectSecp256k1HdWallet.fromMnemonic(config.mnemonicKeys.tca, {
    prefix: config.networkInfo.walletPrefix,
  });
  ...
  console.log(
    `Using ${chalk.cyan(loopswapTokenCode)} as loopSwap (cw20) Token Code`
  );
  console.log(`Using ${chalk.cyan(loopswapPairCode)} as loopSwap Pair Code`);
  console.log(
    `Using ${chalk.cyan(loopswapFactory)} as loopSwap Factory contract`
  );
  console.log(
    `Using ${chalk.cyan(loopswapFarming)} as loopSwap Farming contract`
  );

  console.log(
    `Using ${chalk.cyan(loopswapLoopTokenContract)} as loopSwap LOOP Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapLoopJunoPairContract
    )} as loopSwap LOOP/JUNO Swap Pair`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapLoopJunoPairLpToken
    )} as loopSwap LOOP/JUNO Swap Pair LP Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapInitialLoopSupply
    )} as loopSwap Loop Initial Supply`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapLoopLiquidity
    )} as loopSwap LOOP/JUNO Pair LOOP liquidity`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapJunoLiquidity
    )} as loopSwap LOOP/JUNO Pair JUNO liquidity`
  );

  console.log(
    `Using ${chalk.cyan(loopswapMaloTokenContract)} as loopSwap MALO Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloJunoPairContract
    )} as loopSwap MALO/JUNO Swap Pair`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloJunoPairLpToken
    )} as loopSwap MALO/JUNO Swap Pair LP Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapInitialMaloSupply
    )} as loopSwap MALO Initial Supply`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloJunoPairMaloLiquidity
    )} as loopSwap MALO/JUNO Pair MALO liquidity`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloJunoPairJunoLiquidity
    )} as loopSwap MALO/JUNO Pair JUNO liquidity`
  );

  console.log(
    `Using ${chalk.cyan(loopswapKaloTokenContract)} as loopSwap KALO Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapKaloJunoPairContract
    )} as loopSwap KALO/JUNO Swap Pair`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapKaloJunoPairLpToken
    )} as loopSwap KALO/JUNO Swap Pair LP Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapInitialKaloSupply
    )} as loopSwap KALO Initial Supply`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapKaloJunoPairKaloLiquidity
    )} as loopSwap KALO/JUNO Pair KALO liquidity`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapKaloJunoPairJunoLiquidity
    )} as loopSwap KALO/JUNO Pair JUNO liquidity`
  );

  console.log(
    `Using ${chalk.cyan(
      loopswapMaloKaloPairContract
    )} as loopSwap MALO/KALO Swap Pair`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloKaloPairLpToken
    )} as loopSwap MALO/KALO Swap Pair LP Token`
  );
  console.log(
    `Using ${chalk.cyan(
      loopswapMaloKaloPairMaloLiquidity
    )} as loopSwap KALO/JUNO Pair MALO liquidity`
  );
  console.log(
    `Using ${chalk.cyan(
...
export async function startSetupLoopVaults(): Promise<void> {
  ...
  await setupLoopVaults(
    ...
    // config
    {
      ...
      loopswap_malo_kalo_pair: loopswapMaloKaloPairContract, // LoopSwap LOOP-JUNO pair contract
      ...
      nativeToken: { cw20: config.networkInfo.nativeToken }, // { native: config.networkInfo.nativeToken },
    }
  );
}
