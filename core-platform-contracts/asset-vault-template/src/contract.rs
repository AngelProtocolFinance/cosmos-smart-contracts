...
use crate::state::{CONFIG, Config, InvestmentInfo, Supply, CLAIMS, INVESTMENT, TaxParameters, TOTAL_SUPPLY};

const FALLBACK_RATIO: Decimal = Decimal::one();

// version info for migration info
const CONTRACT_NAME: &str = "asset-vault-template";
...
pub fn instantiate(
    ...
    let config = Config {
        ...
        name: msg.vault_name,
        description: msg.vault_description,
        taxes: TaxParameters {
...
pub fn unbond(
    ...
    let invest = INVESTMENT.load(deps.storage)?;
    let configs = CONFIG.load(deps.storage)?;
    ...
    let tax = amount * configs.taxes.exit_tax;
...
pub fn query_investment(deps: Deps) -> StdResult<InvestmentResponse> {
    ...
    let res = InvestmentResponse {
        ...
        exit_tax: configs.taxes.exit_tax,
        validator: invest.validator,
...
mod tests {
    ...
    fn default_instantiate(tax_percent: u64, min_withdrawal: u128) -> InstantiateMsg {
        InstantiateMsg {
            vault_name: "Coolest Asset Vault".to_string(),
            vault_description: "A wonderful asset vault, that will triple your investments overnight".to_string(),
            account_ledgers_sc: String::from("default-acct-ledger-sc"),
            token_name: "Cool Derivative".to_string(),
            token_symbol: "DRV".to_string(),
            token_decimals: 9,
            validator: String::from(DEFAULT_VALIDATOR),
            unbonding_period: DAY * 3,
            exit_tax: Decimal::percent(tax_percent),
            max_tax: Decimal::percent(100),
            min_tax: Decimal::percent(0),
    ...
    fn proper_instantiation() {
        ...
        assert_eq!(&token.name, &msg.token_name);
        assert_eq!(&token.symbol, &msg.token_symbol);
        assert_eq!(token.decimals, msg.token_decimals);
