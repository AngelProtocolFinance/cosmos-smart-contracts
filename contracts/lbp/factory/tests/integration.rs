...
use angelprotocol_lbp::asset::{AssetInfo, PairInfo, WeightedAssetInfo};
use angelprotocol_lbp::factory::{ConfigResponse, ExecuteMsg, InstantiateMsg, QueryMsg};
...
fn store_factory_code(app: &mut App) -> u64 {
    let pair_contract = Box::new(
        ContractWrapper::new(
            angelprotocol_lbp_factory::contract::execute,
            angelprotocol_lbp_factory::contract::instantiate,
            angelprotocol_lbp_factory::contract::query,
        )
        .with_reply(angelprotocol_lbp_factory::contract::reply),
    );
...
fn store_pair_code(app: &mut App) -> u64 {
    let pair_contract = Box::new(
        ContractWrapper::new(
            angelprotocol_lbp_pair::contract::execute,
            angelprotocol_lbp_pair::contract::instantiate,
            angelprotocol_lbp_pair::contract::query,
        )
        .with_reply(angelprotocol_lbp_pair::contract::reply),
    );
...
fn store_token_code(app: &mut App) -> u64 {
    let terra_swap_token_contract = Box::new(ContractWrapper::new(
        angelprotocol_lbp_token::contract::execute,
        angelprotocol_lbp_token::contract::instantiate,
        angelprotocol_lbp_token::contract::query,
    ));
...
fn create_and_register_pair_with_reply() {
    ...
    let start_time = SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let end_time = Some(start_time + 1000);
    let owner = "owner0000";
