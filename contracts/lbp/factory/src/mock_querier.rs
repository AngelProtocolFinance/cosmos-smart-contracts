use angelprotocol_lbp::asset::PairInfo;
use angelprotocol_lbp::pair::QueryMsg;
...
pub struct WasmMockQuerier {
    ...
    lbp_pair_querier: LBPPairQuerier,
}

#[derive(Clone, Default)]
pub struct LBPPairQuerier {
                ...
                => match from_binary(&msg).unwrap() {
                    QueryMsg::Pair {} => {
                       let pair_info: PairInfo =
                        match self.lbp_pair_querier.pairs.get(&Addr::unchecked(contract_addr)) {
                            Some(v) => v.clone(),
                            None => {
...
impl WasmMockQuerier {
    pub fn new(base: MockQuerier<Empty>) -> Self {
        WasmMockQuerier {
            base,
            lbp_pair_querier: LBPPairQuerier::default(),
        }
    }

    // configure the angelprotocol-lbp pair
    pub fn with_angelprotocol_lbp_pairs(&mut self, pairs: &[(&Addr, &PairInfo)]) {
        self.lbp_pair_querier = LBPPairQuerier::new(pairs);
    }
