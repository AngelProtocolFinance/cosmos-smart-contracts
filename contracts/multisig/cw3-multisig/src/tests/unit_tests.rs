...
fn percentage_handles_group_changes() {
    ...
    // 50% required, which is 8 of the initial 15
    let voting_period = Duration::Time(20000);
    ...
    // admin changes the group (3 -> 0, 2 -> 7, 0 -> 15) - total = 32, require 16 to pass
    let newbie: &str = "newbie";
    let update_msg = cw4_group::msg::ExecuteMsg::UpdateMembers {
        remove: vec![APTEAM3.into()],
        add: vec![member(APTEAM2, 15), member(newbie, 7)],
    };
    app.execute_contract(Addr::unchecked(OWNER), guardian_group, &update_msg, &[])
        .unwrap();
    ...
    // with updated weights, it would be 3 + 7 = 10 / 16 => Open
    let yes_vote = ExecuteMsg::Vote {
        proposal_id,
        vote: Vote::Yes,
    };
    ...
    // APTEAM1 votes
    // with updated weights, it would be 3 + 7 + 1 = 11 / 16 => Open
    let yes_vote = ExecuteMsg::Vote {
        proposal_id,
        vote: Vote::Yes,
    };
    app.execute_contract(Addr::unchecked(APTEAM1), flex_addr.clone(), &yes_vote, &[])
        .unwrap();
    assert_eq!(prop_status(&app), Status::Open);

    // APTEAM5 votes
    // with updated weights, it would be 3 + 7 + 1 + 5 = 16 / 16 => Passed
    let yes_vote = ExecuteMsg::Vote {
        proposal_id,
        vote: Vote::Yes,
    };
    app.execute_contract(Addr::unchecked(APTEAM5), flex_addr.clone(), &yes_vote, &[])
        .unwrap();
    assert_eq!(prop_status(&app), Status::Passed);

    // new proposal cannot be passed single-handedly by newbie (15 / 32 -> Open)
    let proposal = pay_somebody_proposal();
    let res = app
    ...
    let prop: ProposalResponse = app
        ...
        .query_wasm_smart(&flex_addr, &query_prop)
        .unwrap();
    assert_eq!(Status::Open, prop.status);
}
